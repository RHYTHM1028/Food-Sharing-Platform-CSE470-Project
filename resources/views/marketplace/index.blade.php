...
<div class="bg-gray-100 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 class="text-3xl font-bold text-gray-900 mb-8">Food Marketplace</h1>
        
        <!-- Filters -->
        <div class="bg-white rounded-lg shadow-sm p-4 mb-8">
            <form action="{{ route('marketplace') }}" method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div>
                ...
                @foreach($listings as $listing)
                    <div class="bg-white rounded-lg shadow overflow-hidden">
                        ...
                        <div class="p-4">
                            ...
                            <p class="mt-1 text-sm text-gray-500 line-clamp-2">{{ $listing->description }}</p>
                            <div class="mt-3 flex items-center">
                                <p class="text-sm font-medium text-gray-500 line-through">৳{{ number_format($listing->original_price, 2) }}</p>
                                <p class="ml-2 text-lg font-bold text-primary-600">৳{{ number_format($listing->discounted_price, 2) }}</p>
                                <p class="ml-2 text-sm text-green-600">
                                    {{ round((($listing->original_price - $listing->discounted_price) / $listing->original_price) * 100) }}% off
                                </p>
                            </div>
                            <div class="mt-3 flex items-center text-sm text-gray-500">
                                ...
                                <span>{{ $listing->address['city'] ?? 'Location not specified' }}</span>
                            </div>
                            <div class="mt-2 flex items-center text-sm text-gray-500">
                                <svg class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
                                </svg>
                                <span>Expires: {{ $listing->expiry_date->format('M d, Y') }}</span>
                            </div>
                            <div class="mt-4">
                                <a href="{{ route('listings.show', $listing->id) }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                                    View Details
                                </a>
                            </div>
                        </div>
